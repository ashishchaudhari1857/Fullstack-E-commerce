...
function Cards({ data, AddToCart, DeleteProduct, UpdateProduct, BuyProduct }) {
  return (
    <div className="max-w-md p-2 mx-auto overflow-hidden rounded-md shadow-md ">
      ...
      <div className="p-2">
  <h1 className="mb-2 text-2xl font-semibold text-blue-800 capitalize">{data?.name}</h1>
  <p className="mb-2 text-gray-700"><span className="font-semibold">Price:</span> <span className='text-green-600 font-bold'>${data?.price}</span></p>
  <p className="mb-2 text-gray-700"><span className="font-semibold">Category:</span> <span className='text-purple-500'>{data?.category}</span></p>
  <p className="mb-4 text-gray-700"><span className="font-semibold">Stock:</span> <span>{data?.stock}</span></p>
</div>
